...
function createSagaCore({ reducer, reducers, sagas, initializer }) {
  ...
  const reduxMiddleware = redux.applyMiddleware(sagaMiddleware);
  let storeReducer = (state = {}) => state;
  storeReducer = reducers ? redux.combineReducers(reducers) : storeReducer;
  storeReducer = reducer || storeReducer;
  const store = redux.createStore(storeReducer, reduxMiddleware);
